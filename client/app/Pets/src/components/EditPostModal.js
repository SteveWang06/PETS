...
import { getPostKindFromDataBase, handleUpdatePost } from "../services/requester/UserRequester";
...
const EditPostModal = ({
  ...
  const deleteImage = (uri) => {
    // Tạo một bản sao mới của danh sách ảnh
    const updatedImages = [...images];
    
    // Tìm và xoá hình ảnh có URI tương ứng
    const index = updatedImages.findIndex((image) => image === uri);
    if (index !== -1) {
      updatedImages.splice(index, 1);
      setImages(updatedImages); // Cập nhật danh sách ảnh mới
    }
